classdef NMPC3dem < handle
    ...
    methods
        function obj = NMPC3dem(X_, goal_pos_, n_z_, field_Mat_)
            obj.max_iteration = obj.u_size*obj.N_step;
            obj.U=zeros(obj.max_iteration, 1);
            obj.X=X_;
            obj.goal_pos=goal_pos_;
            obj.n_z = n_z_;
            obj.field_Mat = field_Mat_;
            obj.save_x=[X_(1,1), X_(2,1), n_z_];
            for i = 0:obj.N_step-1
                obj.U((obj.u_size*i)+1, 1)=0.05;
                obj.U((obj.u_size*i)+2, 1)=0.05;
            end
        end
        %{
        ...
        function updateState(obj, U_, dt_)
            ...
            obj.n_z=obj.field_info(obj.X(1, 1), obj.X(2, 1));
            x_=round(obj.X(1,1)*10)/10;
            y_=round(obj.X(2,1)*10)/10;
            if x_==0
                x_=0.1;
            end
            if y_==0
            ...
            %obj.n_z=obj.field_info(x_, y_);
            %disp([obj.field_info(x_, y_) x_ y_])
            %obj.n_z=obj.field_Mat(x_*10, y_*10);
            X_=[obj.X(1,1), obj.X(2,1), obj.n_z];
            %disp("U_")
            %disp(U_)
            %disp("U_end")
            disp([x_ y_])
            %disp([obj.n_z obj.field_Mat(round(4000*x_+y_*10-400),3)])
            %disp([obj.X(1,1), obj.X(2,1), obj.n_z]);
            obj.save_x=[obj.save_x;X_];
        ...
        function rhrx = rHrx(obj, x_, u_, lamda_)
            ...
            lamda1_=lamda_(1, 1);
            lamda2_=lamda_(2, 1);
            %z_=power((obj.field_info(x1_, x2_)-obj.n_z), 2);
            x_=round(x1_*10)/10;
            y_=round(x2_*10)/10;
            if x_==0
                x_=0.1;
            ...
            %disp(["a" x1_ x2_ x_ y_ obj.field_info(x1_, x2_) obj.field_info(x_, y_)])
            zd_=15*abs(obj.field_info(x1_, x2_)-obj.n_z);
            %disp([x_ y_ 4000*x_+y_*10-400])
            %disp(round(4000*x_+y_*10-400))
            %disp(obj.field_Mat(round(4000*x_+y_*10-400),3))
            %zd_=15*abs(obj.field_Mat(x_*10, y_*10)-obj.n_z);
            %disp(obj.field_info(x1_, x2_));
            %disp([x1_, x_, x2_, y_,  z_, zd_, obj.field_info(x1_, x2_), obj.n_z])
            %disp((x1_-obj.goal_pos(1, 1)))
            rhrx=[(x1_-obj.goal_pos(1, 1))+zd_, (x2_-obj.goal_pos(2, 1))+zd_, x3_-obj.goal_pos(3, 1)-lamda1_*u1_*sin(x3_)+lamda2_*u1_*cos(x3_)];
            %rhrx=[(x1_-obj.goal_pos(1, 1))+z_, (x2_-obj.goal_pos(2, 1))+z_, x3_-obj.goal_pos(3, 1)-lamda1_*u1_*sin(x3_)+lamda2_*u1_*cos(x3_)];
        end
        function cgmres = CGMRES(obj, time_, goal_pos_, field_info_)
            obj.dt=obj.tf*(1-exp(-obj.alpha*time_))/obj.N_step;
            obj.goal_pos=goal_pos_;
            obj.field_info = field_info_;
            %gmres法を用いてdUを求める
